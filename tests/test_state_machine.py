...
def test_state_machine_transition_whitelist():
    ...
    # 合法路径断言
    assert (
        sm.can_transition_to(AgentState.REQUIREMENT_EXTRACTION, AgentState.REQUIREMENT_ANALYSIS)
        is True
    )
    ...
    # 非法越级跃迁拦截断言
    assert (
        sm.can_transition_to(AgentState.REQUIREMENT_EXTRACTION, AgentState.DELIVERY_COMPLETED)
        is False
    )
...
def test_healing_max_retries_circuit_breaker():
    ...
    assert final_state == AgentState.HUMAN_INTERRUPT, (
        f"3次失败后必须熔断为 HUMAN_INTERRUPT，实际为: {final_state}"
    )
